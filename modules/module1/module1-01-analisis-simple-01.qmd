...
download.file("https://open.neotomadb.org/R-workshop/webr_repo/src/generateHTML.R", destfile = "plotLeaflet.R", mode = "wb")
...
sa_sites <- neotoma2::get_sites(loc = sa$sf)
```
...
#| exercise: true
#| context: interactive
head(neotoma2::summary(sa_sites))
```
...
```{webr-r}
#| label: display head sa_datasets
#| exercise: true
#| context: interactive
head(datasets(sa_datasets))
```
...
head(neotoma2::summary(sa_pollen))
```
